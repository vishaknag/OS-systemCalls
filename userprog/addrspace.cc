...
AddrSpace::AddrSpace(OpenFile *executable) : fileTable(MaxOpenFiles) {
	...
	//bitMapObject->Clear(numPages);   // Changed
	phyPageNo = bitMapObject->Find();
		if(phyPageNo != -1)
		{
			pageTable[i].physicalPage = phyPageNo;
			bzero(&(machine->mainMemory[(phyPageNo * PageSize)]), PageSize);
		}	
		else
		{
		...
		{	
		    //Copying one page at a time to main memory from address mail 
...
TranslationEntry* AddrSpace::GetPageTableRef()
{
	return pageTable;
}
...
void AddrSpace::SetPageTableRef(TranslationEntry *newPageTable)
{
